...
- **Frame number consistency between OpenCV seeking and ffmpeg sequential decode** (2026-05-17)
  ...
  - These two counting methods agree as long as every frame in the container has a unique, monotonically increasing pts and stream's average fps is exactly correct and stable. If any frames are dropped, duplicated, or have non-monotonic pts (possible with some camera firmware or re-muxed files) or there is variation in frame rate, the two counters will diverge, causing LED-detected sync events to be attributed to the wrong frame numbers in the sync_points table.
...
- **Performance: tracker and visualization are too slow for long sequences** (2026-03-28)
  ...
  - **New data point (2026-07-06)**: the same tracker binary runs at ~5 fps built natively with MSVC (`optbuild/`) vs. ~7-10 fps built via MinGW cross-compile from WSL, on the same machine/hardware. Neither build passes any explicit `-march`/`-mavx`/`/arch:` flag (checked in `meson.build` and `cross/mingw-w64-x86_64.ini`), so this isn't an instruction-set mismatch. Most likely explanation: meson's `buildtype=release` maps to `-O3` on GCC but only `/O2` on MSVC (no MSVC equivalent of `-O3`), and MSVC's optimizer is well documented to be weaker than GCC's specifically on heavy expression-template code like Eigen. Not yet profiled or acted on — noted for whenever tracker performance work happens. Things worth trying first: `-Db_lto=true` (whole-program optimization, one meson flag), `/arch:AVX2` (would need the equivalent enabled on the GCC/MinGW side too for a fair comparison), possibly `/fp:fast`.
...
- **Background `posetrak-tracker` runs killed after ~20-25 min, cause unknown** (2026-07-16)
  - A multi-person tracking run (3 people, `--session-db`, `--smooth`, cross-person anchoring at 400mm) launched via a Claude Code background Bash task was killed twice in a row, both times at almost the identical progress point (~1430-1500/3403 frames, ~42-44%) with no crash signature: no error printed, no Windows Error Reporting entry, the process simply stopped existing. No explicit timeout was set on either invocation.
  - The consistent stopping point across two independent runs points to a wall-clock-based kill (~20-25 min elapsed) rather than a tracker bug, resource exhaustion, or data issue — but the exact mechanism is unknown. Candidates, none confirmed: a background-task duration cap in the Claude Code harness/session infrastructure; something in this machine's process/session management. Not a tracker crash — CPU/RAM usage for this workload is modest, and a short (8s) slice of the same command with the same config completed cleanly end-to-end.
  - Workaround: run long tracker invocations directly in a standalone shell (outside any Claude Code background task) rather than backgrounded through the agent.
  - Chunking the time range as an alternative workaround was considered and rejected: `Tracker::initialize()` always re-triangulates and re-solves IK from scratch (no checkpoint/restore mechanism — see the cross-person plan doc's explicitly-deferred "Tracker checkpoint/restore API" item), so running in pieces would produce independently-converged, discontinuous segments rather than one continuous track.

- **`MultiPersonTracker::run()` is single-threaded; per-person parallelism during non-contact frames is unrealized** (2026-07-16)
  - `src/tracking/multi_person_tracker.cpp:1011-1027`'s per-frame loop (`for step: for person(in rotating order): step_person_context(...)`) is plain sequential C++ — no `std::thread`/`std::async` anywhere in the file. Observed on a 3-person cross-person run: CPU load sat at ~30-66%, consistent with a single-core workload on a multi-core machine.
  - This is a known, explicitly-deferred optimization, not an oversight: `docs/roadmap/features/error-improvements/phase5-cross-person-plan.md`'s Stage 1 section says persons are "fully independent outside contact windows (each owns its own Pinocchio model/data, so per-instance thread safety is clean), and running them on threads for non-contact frames is a near-free ~N× later; don't bake the sequential interleave into deep call structure" — the architecture was kept loose enough to add this, but it was never implemented.
  - Why it's not a trivial thread-pool wrap: during an active contact window, person B's step at frame *t* is meant to see person A's already-computed frame-*t* posterior (Stage 2's rotating-order design, mitigating the "who-goes-first misses this frame's anchor" asymmetry). Naively threading all persons would race on that dependency. `update_contact_gate()` already tracks which frames have active pairs, so the information needed to decide "safe to thread this frame" exists but isn't wired to any parallel execution.
  - Potential approach: parallelize person-stepping only on frames with no active contact pairs; serialize (as today) whenever any pair is active that frame.
