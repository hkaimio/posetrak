...
posetrak marker-body calibrate-video --video PATH --intrinsics ID \
  ...
  # come from the camera track. The video is not a capture: --intrinsics is the id of an intrinsics
  # calibration (`calib list`) of the camera in the mode that filmed it, looked up in the session and
  # then the registry; the video must have the image size it was calibrated for.
```
