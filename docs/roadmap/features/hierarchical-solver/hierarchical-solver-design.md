...
**PR 6's two prerequisites are done** (`freeflyer_joint`/`ref_marker`
skeleton metadata + the dual-group-membership fix; the skeleton
`groups:` converter script and its application to every tracked
skeleton YAML) — both surfaced real, previously-undiagnosed gaps
(dual membership wasn't representable at all; the converter's first
version corrupted a differently-topologized test fixture) rather than
being pure plumbing, which is why they needed their own review
checkpoints before landing. PR 6 itself (CLI/config plumbing:
existence-based hierarchical toggle, per-stage tuning resolution) is
next, followed immediately by PR 7 (integration test) using a real
production tracking run + BVH export as the validation, per Harri's
2026-07-21 request.
