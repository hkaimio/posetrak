...
**Status (2026-07-20): PRs 1, 2, and 3 done, PR 4's schema half done,
paused before `ResultWriter`'s RMW capability.** Not a UI-verification
...
without running it. `ResultWriter`'s RMW work is next: its exact shape
(what a "patch" call looks like) now has a concrete answer from PR 3's
`initialize_with_fixed_root()`/child-layout design.
