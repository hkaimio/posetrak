...
| **1d** — Finalisation + manifest | `pose_sequence_keypoints` manifest table (§4.3); finalise pipeline extended to emit one object `pose_observation_sequence` (+ manifest rows) per trial directly from the 1c detection run — automatic/trivial for an object, unlike a person run, since there is no track-to-person stitching decision to make first (§7.1 ordering note below). | Finalise a real object detection run; inspect the resulting sequence's manifest names (`<marker>:c0`..`c3` per §4.3) against the marker body definition, and confirm blob shapes/landmark count match; run the existing finalisation consistency checks (whatever already validates person sequences) against the new object sequence. |
| **1e** — ObjectPanel review | New `ObjectPanel` (sibling of `PersonPanel`, §6.2 step 3): crop grid with detected corners overlaid, scrubber, and the *same* `pose_observation_edits` keypoint-edit mechanism `PersonCropGridWidget` already uses for fixing bad frames — reused directly rather than a second, pre-finalisation editing path (§7.1 ordering note below). | Open a real finalised object sequence (from 1d) in the panel; corners visibly overlay correctly across cameras and frames; deliberately corrupt one frame's corner via the edit UI, confirm the correction persists across a panel reload and is what a later tracking run would actually read. |
| **1f** — Tracker: multi-source load + rigid init | C++ `input_tracks` binding, `(track, landmark)` → marker-index resolution via the manifest, `PersonSpec`'s track map, multi-source `SessionReader::load_observations()` (single-track case); rigid Kabsch/Umeyama initialization path in `Tracker::initialize()` (algorithms §4.2). | Run `posetrak-tracker track` against the 1e-reviewed sequence with the 1b-generated skeleton on a real prop-only trial (or a synthetic rigid-trajectory sequence per the algorithms-doc §7 test pattern); assert successful init (residual RMS within the existing retry threshold) and a plausible 6-DOF trajectory written to `tracking_results`/`root_pose.csv`. This is phase 1's actual finish line — first end-to-end tracked prop. |

**1d/1e ordering note** (discovered during 1d's own implementation, 2026-08-30): the original draft put review before finalisation, mirroring the wording of §6.2's UX steps. But for a *person*, "review before finalising" is real: pre-finalisation review is track-to-person **stitching** (`PoseExtractionWindow`), a genuine decision, and only *post*-finalisation review (`PersonPanel`/`PersonCropGridWidget`, via `pose_observation_edits`) does per-frame keypoint correction — no pre-finalisation per-frame correction path exists anywhere in the codebase today, for anyone. An object's own phasing already established "no stitching step" (one object, one track, identity is a given), which removes the only reason a pre-finalisation stage would need to exist at all — so finalisation for an object is immediate/automatic, and review moves *after* it, reusing the exact same `pose_observation_edits` mechanism `PersonCropGridWidget` already has (`write_observation_edit`/`update_single_keypoint_edit`/`read_observations_with_edits` are already keypoint-count-agnostic, not person-specific) instead of building a second, parallel correction path for raw `detection_keypoints` that nothing else in the project has ever needed.
