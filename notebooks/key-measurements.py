...
def _(Path, fk_rest_pose, mo, np, skeleton_input, yaml):
    ...
    if not _p.exists():
        ...
        _jp = fk_rest_pose(_skel.get("joints", []))

        def _d(a, b):
            return float(np.linalg.norm(_jp[a] - _jp[b]))

        def _mid(a, b):
            return (_jp[a] + _jp[b]) / 2.0
        ...
        tmpl_ref = {
            ...
            "head":           float(np.linalg.norm(_jp["head"] - _mid("shoulder.L", "shoulder.R"))),
        }
    return (tmpl_ref,)
