...
def _(Path):
    ...
    result_dir = Path("/home/harri/projects/posetrak/tracking_tests/cp2-calibrate-timo")
    return (result_dir,)
...
@app.cell
def _(px, tracking_stats):
    _tracking_stats_long = tracking_stats[80:].melt(
            id_vars=["frame"],
...
@app.cell
def _(pd, px, result_dir):
    state_vectors_df = pd.read_csv(result_dir / "state_vectors.csv")

    _scale_group_cols = [
        col for col in state_vectors_df.columns
        if col.startswith("scale_group") and not col.endswith("_velocity")
    ]

    _state_vectors_melted = state_vectors_df.melt(
        id_vars=["tracker_frame_idx"],
        value_vars=_scale_group_cols,
        var_name="scale_group_metric",
        value_name="scale_value",
    )

    _scale_group_plot = px.line(
        _state_vectors_melted,
        x="tracker_frame_idx",
        y="scale_value",
        color="scale_group_metric",
        title="Scale Group Values Over Frames",
        labels={
            "tracker_frame_idx": "Frame Index",
            "scale_value": "Scale Value",
            "scale_group_metric": "Scale Group",
        },
        hover_data=["tracker_frame_idx", "scale_group_metric", "scale_value"],
    )

    _scale_group_plot.update_layout(hovermode="x unified")
    _scale_group_plot
    return
