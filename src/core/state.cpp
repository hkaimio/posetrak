...
State::State(int num_dof)
    : root_position_(Eigen::Vector3d::Zero()),
      root_orientation_(Eigen::Quaterniond::Identity()) {
    if (num_dof < 0) {
...
State::State(Eigen::Vector3d const& root_position,
...
Eigen::Quaterniond State::axis_angle_to_quaternion(Eigen::Vector3d const& axis_angle) {
    ...
    return Eigen::Quaterniond(cos_half, sin_half * axis.x(), sin_half * axis.y(), sin_half * axis.z());
}
...
nlohmann::json State::to_json() const {
    ...
    j["root_orientation"] = {root_orientation_.w(), root_orientation_.x(), 
                             root_orientation_.y(), root_orientation_.z()};
    
    // Joint angles
    j["joint_angles"] = std::vector<double>(joint_angles_.data(), 
                                            joint_angles_.data() + joint_angles_.size());
    
    // Velocities
    j["root_velocity"] = {root_velocity_(0), root_velocity_(1), root_velocity_(2)};
    j["joint_velocities"] = std::vector<double>(joint_velocities_.data(),
                                                 joint_velocities_.data() + joint_velocities_.size());
